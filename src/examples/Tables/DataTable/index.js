...
function DataTable({
  ...
  const handleRowDoubleClick = async (row) => {
    //debugger;
    if (row.original.fileType === "REC") {
      await fetchRegularOrderDetails(row.original.uid);
      setIsDataLoaded(true);
    } else if (row.original.fileType === "CRE") {
      await fetchCreditOrderDetails(row.original.uid);
      setIsDataLoaded(true);
    } else if (
      row.original.fileType === "ORD REG" ||
      row.original.fileType === "ORD REG#" ||
      row.original.fileType === "ORD ADJ" ||
      row.original.fileType === "ADV ORD"
    ) {
      //debugger;
      await fetchAllOrderDetails(row.original.uid);
      setIsDataLoaded(true);
    }
  };
  ...
  useEffect(() => {
    debugger;
    if (isDataLoaded && regularOrderDetails && Object.keys(regularOrderDetails).length > 0) {
      // Serialize the regularOrderDetails object
      const serializedDetails = encodeURIComponent(JSON.stringify(regularOrderDetails));

      // Open the new tab with the serialized data in the URL
      console.log("regularOrderDetails in new Form", regularOrderDetails);
      const route = `/edit-credit-header?details=${serializedDetails}`;
      window.open(route, "_blank");

      setIsDataLoaded(false); // Reset the flag
    } else if (isDataLoaded && creditOrderDetails && Object.keys(creditOrderDetails).length > 0) {
      // Serialize the regularOrderDetails object
      const serializedDetails = encodeURIComponent(JSON.stringify(creditOrderDetails));

      // Open the new tab with the serialized data in the URL
      console.log("creditOrderDetails in new Form", creditOrderDetails);
      const route = `/edit-credit-header?details=${serializedDetails}`;
      window.open(route, "_blank");
      setIsDataLoaded(false); // Reset the flag
    } else if (isDataLoaded && allOrderDetails && Object.keys(allOrderDetails).length > 0) {
      // Serialize the regularOrderDetails object
      const serializedDetails = encodeURIComponent(JSON.stringify(allOrderDetails));

      // Open the new tab with the serialized data in the URL
      console.log("allOrderDetails in new Form", allOrderDetails);
      const route = `/edit-credit-header?details=${serializedDetails}`;
      window.open(route, "_blank");

      setIsDataLoaded(false); // Reset the flag
    }
  }, [isDataLoaded, regularOrderDetails, creditOrderDetails, allOrderDetails]);
                              ...
                              <TableRow>
                                {nestedColumns.map((nestedCol, index) => (
                                  <DataTableHeadCell
                                    ...
                                    style={{ color: "white" }}
                                    sorted={setSortedValue(nestedCol)}
                                  >
                                    {nestedCol.Header}
                                  </DataTableHeadCell>
                                ))}
...
DataTable.propTypes = {
  ...
  filterTableData: PropTypes.shape({
    columns: PropTypes.array,
    rows: PropTypes.array,
  }),
  ...
  fetchRegularOrderDetails: PropTypes.func.isRequired,
  fetchCreditOrderDetails: PropTypes.func.isRequired,
  regularOrderDetails: PropTypes.object.isRequired,
  creditOrderDetails: PropTypes.object.isRequired,
  fetchAllOrderDetails: PropTypes.object.isRequired,
  allOrderDetails: PropTypes.object.isRequired,
